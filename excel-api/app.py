    ...
    for dia in r["dias"]:
        ...
        for ei, ej in enumerate(dia["ejercicios"]):
            ...
            for i, (sc, kc, is_desc, num) in enumerate(micro_cols):
                if is_desc:
                    rir_m = rir_val + 1; bg_m = C_ORANGE_LIGHT
                elif i >= MICROS - 2:
                    rir_m = max(0, rir_val - 1); bg_m = C_BLUE_MED
                else:
                    rir_m = rir_val; bg_m = C_BLUE_LIGHT if i % 2 == 0 else C_WHITE
                cs = ws.cell(row=row, column=sc, value=f"{ej['series']}x? Rir{rir_m}")
