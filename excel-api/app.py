...
def build_excel(r):
    ...
    for dia in r["dias"]:
        ...
        for ei, ej in enumerate(dia["ejercicios"]):
            ...
            # Build series label - each serie keeps its own RIR
            def build_label(series_data, rir_delta=0):
                if isinstance(series_data, list):
                    ...
                    for s in series_data:
                        ...
                        num_s = s.get("series", "")
                        reps  = s.get("reps", "")
                        rir   = max(0, int(s.get("rir", "0")) + rir_delta)
                        if tipo:
                            parts.append(f"{tipo} {num_s}x{reps} Rir{rir}")
                        else:
                            parts.append(f"{num_s}x{reps} Rir{rir}")
                    return " / ".join(parts)
                else:
                    rir_v = int(ej.get("rir","0")) if str(ej.get("rir","0")).isdigit() else 0
                    return f"{series_data}x? Rir{max(0, rir_v + rir_delta)}"

            series_data = ej.get("series", [])

            for i, (sc, kc, is_desc, num) in enumerate(micro_cols):
                if is_desc:
                    cell_val = build_label(series_data, rir_delta=1)
                    bg_m = C_ORANGE_LIGHT
                elif i >= MICROS - 2:
                    cell_val = build_label(series_data, rir_delta=-1)
                    bg_m = C_BLUE_MED
                else:
                    cell_val = build_label(series_data, rir_delta=0)
